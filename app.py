...
LANGUAGES = {
    ...
    "fr": {
        ...
        "nav_home": "Accueil",
        "nav_analyze": "Analyser une image",
        "nav_history": "Résultats précédents",
        "nav_info": "Infos maladie",
    ...
    "en": {
        ...
        "badge_support_desc": "Contact our support team",
        "nav_home": "Home",
        "nav_analyze": "Analyze Blood Image",
        "nav_history": "Previous Results",
        "nav_info": "Disease Info",
...
with st.sidebar:
    ...
    <div class="sidebar-glass">
        <h4>📋 القائمة / Menu</h4>
        <div class="nav-item active" onclick="st.session_state.page='home'">🏠 {t('nav_home')}</div>
        <div class="nav-item" onclick="st.session_state.page='analyze'">🔬 {t('nav_analyze')}</div>
        <div class="nav-item" onclick="st.session_state.page='history'">📊 {t('nav_history')}</div>
        <div class="nav-item" onclick="st.session_state.page='info'">ℹ️ {t('nav_info')}</div>
        <div class="nav-item" onclick="st.session_state.page='health'">💡 {t('nav_health')}</div>
        <div class="nav-item" onclick="st.session_state.page='about'">📱 {t('nav_about')}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div class="sidebar-glass">
        <h4>{t('sidebar_doctor_title')} <span class="coming-badge">{t('hero_badge')}</span></h4>
...
if st.session_state.page == "home":
    ...
    <div class="hero">
        ...
        <h1>{t('hero_title')}</h1>
        <p>{t('hero_desc')}</p>
        <button class="hero-btn" onclick="st.session_state.page='analyze'">{t('hero_btn')}</button>
...
# ========== PAGE: ANALYSE ==========
    ...
    st.markdown(f'<div style="font-size:28px; font-weight:700; color:#0f172a; margin-bottom:1rem;">{t("upload_title")}</div>', unsafe_allow_html=True)
    ...
    if uploaded is not None:
        ...
        <div style="background:rgba(255,255,255,0.6); backdrop-filter:blur(8px); border-radius:28px; padding:3rem; text-align:center; box-shadow:0 8px 32px rgba(0,0,0,0.04); margin-top:2rem; border:1px solid rgba(255,255,255,0.3); animation:fadeUp 0.8s ease;">
            ...
            <h3 style="color:#0f172a;">{t('welcome_title')}</h3>
            <p style="color:#64748b;">{t('welcome_desc')}</p>
        </div>
        """, unsafe_allow_html=True)
...
# ========== PAGE: INFOS MALADIE ==========
    ...
    st.markdown(f'<div class="section-title">{t("nav_info")}</div>', unsafe_allow_html=True)
    st.markdown("""
    <div style="background:rgba(255,255,255,0.6); backdrop-filter:blur(8px); border-radius:24px; padding:2rem; border:1px solid rgba(255,255,255,0.3);">
        <h4>🩸 Qu'est-ce que l'anémie ?</h4>
        <p>L'anémie est une condition où le nombre de globules rouges ou la quantité d'hémoglobine dans le sang est inférieur à la normale.</p>
        ...
        <ul>
            <li>Carences en fer, vitamine B12 ou folate</li>
...
# ========== PAGE: CONSEILS SANTÉ ==========
    ...
    st.markdown(f'<div class="section-title">{t("nav_health")}</div>', unsafe_allow_html=True)
    st.markdown("""
    <div style="background:rgba(255,255,255,0.6); backdrop-filter:blur(8px); border-radius:24px; padding:2rem; border:1px solid rgba(255,255,255,0.3);">
        <h4>🥩 Alimentation riche en fer</h4>
        <ul>
            <li>Viande rouge, volaille, poisson</li>
            <li>Légumineuses (lentilles, pois chiches)</li>
            <li>Épinards et légumes verts</li>
            <li>Fruits secs (abricots, raisins)</li>
        </ul>
        <h4>🍊 Améliorer l'absorption</h4>
        <ul>
            <li>Associer le fer avec de la vitamine C (agrumes, kiwi)</li>
            <li>Éviter le thé et le café pendant les repas</li>
        </ul>
        <h4>💧 Hydratation</h4>
        <p>Boire au moins 1,5 litre d'eau par jour.</p>
    </div>
    """, unsafe_allow_html=True)

# ========== PAGE: À PROPOS ==========
elif st.session_state.page == "about":
    st.markdown(f'<div class="section-title">{t("nav_about")}</div>', unsafe_allow_html=True)
    st.markdown("""
    <div style="background:rgba(255,255,255,0.6); backdrop-filter:blur(8px); border-radius:24px; padding:2rem; border:1px solid rgba(255,255,255,0.3);">
        <h4>🩸 AnemiCheck AI</h4>
        <p><strong>Version :</strong> 1.0.0</p>
        <p><strong>Description :</strong> Application de détection d'anémie par intelligence artificielle.</p>
        <p><strong>Technologies :</strong></p>
        <ul>
            <li>U‑Net (ResNet34) pour la segmentation</li>
            <li>EfficientNet‑B3 pour la classification</li>
            <li>Streamlit pour l'interface</li>
        </ul>
        <p><strong>Contact :</strong> support@anemichck.ai</p>
        <p style="font-size:12px; color:#94a3b8; margin-top:1rem;">© 2026 AnemiCheck AI - Tous droits réservés</p>
    </div>
    """, unsafe_allow_html=True)
